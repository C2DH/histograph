  ...
  .controller('GuidedTourCtrl', function ($rootScope, $scope, $log, $timeout, localStorageService, EVENTS) {
    ...
    */
    $scope.loadCursors = function () {
      const cursors = localStorageService.get('guidedtour');
      if (!cursors) {
        $log.log('GuidedTourCtrl -> loadCursors(), filling localstorage ...');
        $scope.saveCursors();
        return
      }
      for (const i in cursors) {
        if ($scope.steps[i]) {
      ...
      if (cursors.enabled) { $scope.enabled = (!!cursors.enabled) || true; }
    ...
    */
    ...
    $scope.previousStep = function () {
      let cursor = Math.max(0, $scope.steps[currentState].cursor - 1);
    ...
    $scope.start = function () {
      ...
      if ($scope.forceStart) { $scope.steps[currentState].cursor = 0; } // clone;
    ...
    */
    ...
    $rootScope.$on(EVENTS.STATE_VIEW_CONTENT_LOADED, function (e, state) {
      ...
      if (__promise) { $timeout.cancel(__promise); }
