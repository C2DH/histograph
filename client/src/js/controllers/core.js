  ...
  .controller('CoreCtrl', function ($scope, $rootScope, $location,
    ...
    */
    ...
    $scope.setHeader = function (key, value) {
      if (typeof key === 'object') {
        for (const i in key) $scope.headers[i] = key[i];
      } else $scope.headers[key] = value;
    }
    ...
    */
    ...
    /*
      ...
      -------------------
     */
    const Trail = function (path, start, index, level) {
      this.paths = [{
        path: path,
        start: start
      }];
    ...
    */
    ...
    $scope.lock = function (key) {
      $log.log('CoreCtrl -> lock() - key:', key || 'no key provided');

      if (key) $scope.loadingQueue[key] = true;

      $scope.isLoading = true;
    }

    $scope.unlock = function (key) {
      if (key) $scope.loadingQueue[key] = false;

      const pending = _.compact(_.values($scope.loadingQueue)).length > 0;
      $log.log('CoreCtrl -> unlock() - key:', key || 'no key provided', '- pending:', $scope.loadingQueue);

      if (!pending) $scope.isLoading = false;
    }
    ...
    */
    ...
    $rootScope.$on('$stateChangeStart', function (e, state) {
      ...
      $scope.lock('$dom');
      // empty
      if (!_.isEmpty($scope.relatedItems)) $scope.relatedItems = [];
      if (!_.isEmpty($scope.timeline)) $scope.timeline = [];
    })
    ...
    $scope.$on(EVENTS.USER_NOT_AUTHENTIFIED, function (e) {
      ...
      } else $scope.unsetMessage(MESSAGES.LOADING);
    });
    ...
    */
    ...
    $scope.queue = function (item, inprog) {
      ...
      const indexOfItemId = $scope.playlistIds.indexOf(+itemId);


      const isAlreadyInQueue = indexOfItemId != -1;
    ...
    $scope.getPlaylistFromLocalStorage = function () {
      let storedItems = localStorageService.get('playlist');
      if (!storedItems) {
    ...
    */
    ...
    $scope.toggleQueue = function (item) {
      if ($scope.queueStatus != 'active') $scope.queueStatus = 'active';
      else $scope.queueStatus = 'sleep';
    }
    ...
    $scope.removeFromQueue = function (item) {
      ...
      for (let i = 0; i < $scope.playlist.length; i++) {
      ...
      if (indexToRemove !== -1) $scope.playlist.splice(indexToRemove, 1);
    ...
    */
    $rootScope.raiseIssue = function (entity, resource, kind, solution, next) {
      ...
      if (resource) params.mentioning = resource.id;

      if (solution) params.solution = !isNaN(solution) ? parseInt(solution) : solution;
    ...
    */
    $scope.signale = function (entity, next) {
      EntityExtraFactory.save({
        ...
        // feedback
        if (res.status == 'ok') $scope.setMessage('thanks for your feedback')
        if (next) next();
      });
    }
