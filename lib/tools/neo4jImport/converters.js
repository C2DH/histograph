...
function wrapAsRelationship(startId, endId, type) {
  return properties => ({
    startId,
    endId,
    type,
...
function createResourcePayloadListToEntitiesAndRelationships(
  ...
  const lastIdsCopy = clone(lastIds)
  const entityTypeAndSlugToIdMappingCopy = clone(entityTypeAndSlugToIdMapping)
      ...
      nodesAndRelationshipsLabels.forEach(l => {
        ...
        if (l === 'entity') {
          ...
          items = items.filter(item => {
            ...
            return !(key in entityTypeAndSlugToIdMappingCopy)
          })

          // add all remaining entities to the mapping
          items.forEach(item => {
            const key = typeAndSlugTag(item.properties, last(without(item.labels, 'entity')))
            entityTypeAndSlugToIdMappingCopy[key] = item.id
          })
        }

        acc[l] = acc[l].concat(items)
        if (includes(nodesLabels, l)) {
