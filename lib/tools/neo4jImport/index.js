...
async function main(baseDir, payloadsPath) {
  ...
  let entityMappingPromise = collectEntityTypeAndSlugToIdMapping(baseDir, csvServices.entity)

  const fileTypes = ['resource', 'appears_in', 'version', 'describes', 'entity']

  const typeAndLastIdPairsPromise = Promise.all(without(fileTypes, 'entity')
    .map(async type => {
      let id = 0
  ...
  let [entityMapping, typeAndLastIdPairs] = await Promise.all([
    entityMappingPromise, typeAndLastIdPairsPromise
  ])
  let lastIds = fromPairs(typeAndLastIdPairs)
  ...
  const outputStreams = fromPairs(await Promise.all(fileTypes.map(async type => {
    ...
    serializer.pipe(fs.createWriteStream(await getFilePath(baseDir, type, tag), opts))
    ...
    await new Promise(resolve => {
      ...
      Object.keys(results).forEach(type => {
        results[type]
          ...
          .forEach(item => {
            const canWrite = outputStreams[type].write(item)
            if (!canWrite) {
              numBlockedStreams += 1
              outputStreams[type].once('drain', () => {
                numBlockedStreams -= 1
