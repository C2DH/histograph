...
async function collectEntityTypeAndSlugToIdMapping(filePath) {
  return new Promise((res, rej) => {
    try {
      const rl = readline.createInterface({
        input: fs.createReadStream(filePath)
      ...
      const mapping = {}
      rl.on('line', line => {
        const item = parseLine(line, 'entity')
...
async function getLastId(filePath, type, csv) {
  const lastLine = await getLastLine(filePath)
  if (lastLine) {
    const item = csv.parseLine(lastLine, type)
    return item.id
  }
  return 0
}
...
async function main(baseDir, payloadsPath) {
  ...
  let entityMapping = collectEntityTypeAndSlugToIdMapping(getFilePath(baseDir, 'entity'))
  const fileTypes = ['resource', 'appears_in', 'version', 'describes']

  const lastIdsList = await Promise.all(fileTypes
    .map(async type => getLastId(
      getFilePath(baseDir, type, await getLastTag(baseDir, type)),
      csv
    )))
  ...
  inputReadline.on('line', line => {
    ...
    Object.keys(results).forEach(type => {
      results[type].forEach(item => {
        outputReadlines[type].write(csv.serializeItem(item, type))
      })
    })
  })
