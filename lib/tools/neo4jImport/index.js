...
/**
 * @typedef {[import('csv-stringify').Stringifier, import('fs').WriteStream]} SerializerAndStream
 *
 * @param {_.Dictionary<CsvService>} csvServices
 * @param {_.Dictionary<SerializerAndStream>} outputStreams
 * @param {_.Dictionary<object[]>} results
 */
async function writeResults(csvServices, outputStreams, results) {
...
function readLines(input, output) {
...
async function main(baseDir, payloadsPath) {
  ...
  const fileTypes = ['resource', 'appears_in', 'entity']
   ...
   * @type {_.Dictionary<SerializerAndStream>}
   */
  ...
  const outputsClosedPromise = new Promise((res, rej) => {
    Object.keys(outputStreams).forEach(type => {
      const stream = outputStreams[type][1]
  ...
  async function processLine(line) {
    ...
    await writeResults(csvServices, outputStreams, result.results)
  ...
  return outputsClosedPromise.then(saveHeaders)
}
