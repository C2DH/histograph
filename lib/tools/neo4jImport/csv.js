...
function parseHeader(headerString) {
  ...
  const items = splitIntoItems(headerString)
  return items.map(i => {
...
class CsvService {
  ...
  parseLine(line) {
    ...
    assert(
      ...
      `Line items (${lineItems}) length is different from rules (${rules}) length for type "${this.schemaUri}"`
    )
  ...
  serializeItem(item) {
    ...
    const [lineItems, updatedRules] = unzip(lineItemsWithRules)
  ...
  serializeHeader() {
    const { rules } = this
    const items = rules.map(([prop, rule, space]) => {
