...
MATCH ()-[a:appears_in]-(r:resource)-[:describes]-(v:version {language: "${languageCode}"})
WHERE NOT exists(a.context__${languageCode})
RETURN {
  resourceUuid: r.uuid,
  contextYaml: v.yaml    
} as result
...
async function getBatchOfResourcesWithNoContexts(languageCode, batchSize, skipSize) {
  return executeQuery(getResourceWithNoContextQuery(languageCode), { limit: batchSize, skip: skipSize })
}
...
async function main(languageCode = 'en', batchSize = 10) {
  let resources = []
  ...
  do {
    ...
    resourcesCounter += resources.length
