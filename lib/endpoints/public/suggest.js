...
const findMentionedEntitiesByNamePatternEndpoint = asyncHandler(async (req, res) => {
  const {
    query,
    skip: skipString,
    limit: limitString,
  ...
  const [limit, skip] = [limitString, skipString]
    .map(v => (v === undefined ? undefined : parseInt(v, 10)))

  const result = await findMentionedEntitiesByNamePattern(query, constraints, { skip, limit })
  res.status(200).json(result)
}, true)
...
router.get('/entities', findEntitiesByNamePatternEndpoint)
router.get('/entities/mentioned', findMentionedEntitiesByNamePatternEndpoint)
