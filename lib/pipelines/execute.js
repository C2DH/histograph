...
const PipelineModulePath = './pipeline'

const LogDir = path.join(get(settings, 'paths.workerLogs', './logs'), 'pipelines')
 ...
 */
async function startLegacyPipelineProcess(task, parameters = {}) {
  ...
  const params = Object.keys(parameters).map(k => {
    const v = parameters[k]
    return `--${k}=${v}`
  })
  const subprocess = fork(
    ManageModulePath,
    [`--task=${task}`].concat(params),
    {
      detached: true,
      stdio: [
        'ipc',
 ...
 * @returns {string} discovery process GUID
 */
async function startPipelineProcess(taskName, parameters = {}) {
  const processId = uuidv1()
  const logFiles = await getLogFiles(processId)
  const [outFile, errFile] = await Promise.all([logFiles.out, logFiles.err].map(f => openFile(f, 'a')))
  const subprocess = fork(
 ...
 */
async function getPipelineProcessLogs(processId) {
  const logFiles = await getLogFiles(processId)
