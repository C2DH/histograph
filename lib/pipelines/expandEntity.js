...
async function getEntityByUuid(uuid) {
  const result = await executeQuery(entityQueries.get_entity, { id: uuid })
  return first(result)
}
...
async function execute(options = {}) {
  ...
  const [{ entity: preparedEntity }] = c2dhNerdResultToEntityAndAppearanceList(
    ['en', { matched_resource: resource }],
  )

  const neo4jEntity = toNeo4jJson(preparedEntity)

  // properties we do not want to update (yet)
  delete neo4jEntity.name
  delete neo4jEntity.slug

  log.info(`Discovered properties for "${entityUuid}": ${JSON.stringify(neo4jEntity)}`)

  await updateEntityProperties(entityUuid, neo4jEntity)
  return entityUuid
}
