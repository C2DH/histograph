...
const {
  ...
  zip, max, fill, first, last,
  mean
} = require('lodash')

function getAggregatedTopicModellingScoreFromResourcesBin(items, aggregationFn = mean) {
  const scoresList = items.map(item => item.scores).filter(s => !isEmpty(s))
  return zip(...scoresList).map(scores => aggregationFn(scores))
}
