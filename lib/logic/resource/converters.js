 ...
 *
 * @return {object} object satisfying `mutations/merge_resource.json` JSON schema
 */
function createResourcePayloadToMergeResource(payload, username) {
  ...
  const {
    date: start_date,
    time: start_time,
  } = reconcileDate(get(resource, 'start_date'), 'YYYY-MM-DD')

  const {
    date: end_date,
    time: end_time,
  } = reconcileDate(get(resource, 'end_date'), 'YYYY-MM-DD')
  ...
  const mergeResourceObject = omitBy({
    ...
    username,
    previous_resource_uuid: get(resource, 'previous_resource_uuid')
  }, isUndefined)

  return validated(mergeResourceObject, 'mutations/merge_resource.json')
}
