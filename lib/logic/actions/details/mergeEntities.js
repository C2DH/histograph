...
async function changeEntityType({
  ...
  return [
    `${result.count} updates have been performed while changing slugs "${slugs.join(', ')}" and types "${types.join(', ')}" into entity ${newSlug} (${newType})`,
    true
  ]
}
