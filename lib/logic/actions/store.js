...
const QueryGetEntityIdentifier = type => {
  const tag = type === undefined ? '' : `:${type}`
  return `
    MATCH (e:entity${tag} { uuid: {entityUuid} })
    RETURN {
      slug: e.slug,
      name: e.name,
      type: last(labels(e))
...
const QueryGetEntitiesIdentifiers = `
  UNWIND {entityUuidList} as entityUuid 
  MATCH (e:entity { uuid: entityUuid })
  RETURN {
    slug: e.slug,
    name: e.name,
...
const QueryFindPerformedActions = `
  MATCH (a:change_action)
  RETURN a
...
async function getEntitiesIdentifiers(entityUuidList) {
  return executeQuery(QueryGetEntitiesIdentifiers, { entityUuidList })
}

async function findPerformedActions(skip, limit) {
  const results = await executeQuery(QueryFindPerformedActions, { skip, limit })
  return results.map(fromNeo4jChangeAction)
}
